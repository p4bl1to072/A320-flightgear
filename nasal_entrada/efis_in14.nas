...
var toggleAltimeterUnits = func(state) {
    ...
    if (useInHg == 1) {
    	...
    	initial_setting_inhg = getprop("/instrumentation/altimeter/setting-inhg");
    } else {
        # Switch to hPa units
        setprop("/instrumentation/altimeter/inhg", 0);  # Disable inHg mode
        screen.log.write("Altimeter switched to hPa.");
	initial_setting_hpa = getprop("/instrumentation/altimeter/setting-hpa");
    }

    last_encoder_position = getprop("/input/arduino/EFIS/encoder-position");
...
# Encoder
var initial_setting_hpa = getprop("/instrumentation/altimeter/setting-hpa");
var initial_setting_inhg = getprop("/instrumentation/altimeter/setting-inhg");
var last_encoder_position = 0;  # Initialize to 0 or whatever is the starting point
...
var handleEncoderMovement = func {
    ...
    var current_encoder_position = getprop("/input/arduino/EFIS/encoder-position");
    var encoder_delta = current_encoder_position - last_encoder_position;
    var use_inhg = getprop("/input/arduino/EFIS/inHG");

    if (use_inhg == 0) {
        # Mode is hPa, update setting-hpa
        var new_setting_hpa = initial_setting_hpa + encoder_delta;
        setprop("/instrumentation/altimeter/setting-hpa", new_setting_hpa);
    } else {
        # Mode is inHg, update setting-inhg
        var new_setting_inhg = initial_setting_inhg + (encoder_delta * 0.01);
